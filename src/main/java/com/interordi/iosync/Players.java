...
public class Players implements Runnable {
    ...
    public World getWorldFromId(String worldName) {
        ...
        switch (worldName.toLowerCase()) {
            ...
            default:
                // if it doesn’t start with "minecraft:", just use the first world as fallback
                if (!worldName.startsWith("minecraft:")) return Bukkit.getWorlds().get(0);
                String stripped = worldName.substring("minecraft:".length());
                World world = Bukkit.getWorld(stripped);
